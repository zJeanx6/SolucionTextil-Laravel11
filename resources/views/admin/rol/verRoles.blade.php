<x-app-layout>
                                ...
                                @foreach ($roles as $rol)
                                    <tr>
                                        ...
                                        <td class="py-2 px-4 border-b border-gray-200 flex">
                                            <div class="mx-auto">
                                                <button class="bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded" onclick="openEditModal({{ $rol->id }}, '{{ $rol->nombre }}')">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded" onclick="openDeleteModal({{ $rol->id }})">
    ...
    <div id="modal-delete-confirm" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md text-center">
                ...
                <p class="mb-4">¿Estás seguro de que deseas eliminar este rol?</p>
    ...
    <div id="modal-edit" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
                <h2 class="text-xl font-bold mb-4">Actualizar Rol</h2>
                <form id="form-edit" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="edit-nombre" class="block text-sm font-medium text-gray-700">Nombre del Rol:</label>
    ...
    <script>
        function openEditModal(id, nombre) {
            ...
            document.getElementById('form-edit').action = '/admin/rol/editar-roles/' + id;
            document.getElementById('modal-edit').classList.remove('hidden');
            document.getElementById('overlay').classList.remove('hidden');
        }

        function openDeleteModal(id) {
            document.getElementById('form-delete').action = '/admin/rol/eliminar-roles/' + id;
