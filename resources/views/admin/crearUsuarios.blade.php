            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('usuarios.store') }}" method="POST" id="form__registerUser">
                        @csrf
                        <h1 class="text-2xl font-bold mb-6">Crea un nuevo usuario</h1>

                        <!-- Usamos grid para la disposición en 2 columnas -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Primera columna -->
                            <div>
                                <div class="mb-4">
                                    <label for="id" class="block text-sm font-medium text-gray-700">Documento:</label>
                                    <input type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" name="id" id="id" placeholder="Documento del Usuario" value="{{ old('id') }}">
                        ...
                        <div class="text-center mt-6">
                            <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded w-1/2" type="submit">Confirmar</button>
                        </div>
                    </form>
                </div>
            </div>
