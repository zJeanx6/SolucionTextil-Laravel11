<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-2xl font-bold mb-6">Lista de Estados</h1>
                    <div class="mb-4 text-right">
                        <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onclick="document.getElementById('modal-create').classList.remove('hidden'); document.getElementById('overlay').classList.remove('hidden')">Agregar Estado</button>
                    </div>
                        ...
                        <table class="min-w-full bg-white border border-gray-200">
                            ...
                            <tbody>
                                @foreach ($estados as $estado)
                                    <tr>
                                        <td class="py-2 px-4 border-b border-gray-200">{{ $estado->nombre }}</td>
                                        <td class="py-2 px-4 border-b border-gray-200 flex">
                                            <div class="mx-auto">
                                                <button class="bg-blue-500 hover:bg-blue-700 font-bold text-white py-1 px-2 rounded" onclick="openEditModal({{ $estado->id }}, '{{ $estado->nombre }}')">Actualizar</button>
                                                <form action="{{ route('estados.destroy', $estado->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div id="modal-create" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
                <h2 class="text-xl font-bold mb-4">Agregar Nuevo Estado</h2>
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
                ...
                <form id="form-edit" method="POST">
                    ...
                    @method('PUT')
                    <div class="mb-4">
                        <label for="edit-nombre" class="block text-sm font-medium text-gray-700">Nombre del Estado:</label>
    ...
    <script>
        function openEditModal(id, nombre) {
            ...
            document.getElementById('form-edit').action = '/admin/estado/editar-estados/' + id;
