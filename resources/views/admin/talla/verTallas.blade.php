<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        <h1 class="text-2xl font-bold">Lista de Tallas</h1>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white border border-gray-200">
                            <thead>
                                <tr>
                                    <th class="py-2 px-4 border-b border-gray-200">Talla</th>
                            ...
                            <tbody>
                                @foreach ($tallas as $talla)
                                    <tr>
                                        <td class="py-2 px-4 border-b border-gray-200">{{ $talla->letra }}</td>
                                        <td class="py-2 px-4 border-b border-gray-200 flex">
                                            <div class="mx-auto">
                                                <button class="bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded" onclick="openEditModal({{ $talla->id }}, '{{ $talla->letra }}')">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button class="bg-red-500 hover:bg-red-700 text-white py-1 px-2 rounded" onclick="openDeleteModal({{ $talla->id }})">
    ...
    <div id="modal-create" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
                <h2 class="text-xl font-bold mb-4">Agregar Nueva Talla</h2>
                <form action="{{ route('tallas.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label for="letra" class="block text-sm font-medium text-gray-700">Letra de la Talla:</label>
                        <input type="text" name="letra" id="letra" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('letra') }}">
                        @error('letra') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
    ...
    <div id="modal-edit" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
                <h2 class="text-xl font-bold mb-4">Actualizar Talla</h2>
                <form id="form-edit" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="edit-letra" class="block text-sm font-medium text-gray-700">Letra de la Talla:</label>
                        <input type="text" name="letra" id="edit-letra" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        @error('letra') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <div class="text-right">
    ...
    <div id="modal-delete-confirm" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md text-center">
                ...
                <p class="mb-4">¿Estás seguro de que deseas eliminar esta talla?</p>
    ...
    <script>
        function openEditModal(id, letra) {
            document.getElementById('edit-letra').value = letra;
            document.getElementById('form-edit').action = '/admin/talla/editar-tallas/' + id;
            document.getElementById('modal-edit').classList.remove('hidden');
            document.getElementById('overlay').classList.remove('hidden');
        }

        function openDeleteModal(id) {
            document.getElementById('form-delete').action = '/admin/talla/eliminar-tallas/' + id;
